...
Proof.
  ...
  - destruct u; destruct v. 
    ...
    + rewrite <- ih; try lia. rewrite <- (ih m); try lia.
      ...
      (map
         (fun '(ui0, vi0) =>
          if
           le' ui0 ui
             (Init.Nat.max (Init.Nat.max (rk u) (rk_fun l)) (Init.Nat.max (rk v) (rk_fun l0)))
          then vi0
          else bot)
         l0) = lub_list (map (fun '(ui0, vi0) => if le' ui0 ui m then vi0 else bot) l0).
      2: { rewrite -> EQ. destruct lub_list; rewrite <- EQ; try done.
           rewrite <- ih. 
           rewrite <- (ih m). auto. lia.
      f_equal.
      eapply map_ext_in.
      move => [uj vj] Inl0.
      move: (In_rk_fun1 Inl0) => h10.
      move: (In_rk_fun2 Inl0) => h20.
      rewrite <- ih; try lia.
    ...
    +       eapply forall_ext.
      ...
      have EQ:  
        lub_list
          (map (fun '(ui0, vi0) => if le' ui0 ui (Init.Nat.max (rk_fun l) (rk_fun l0)) then vi0 else bot)
         l0) = lub_list (map (fun '(ui0, vi0) => if le' ui0 ui m then vi0 else bot) l0).
      2: rewrite EQ; done.
      f_equal.
      eapply map_ext_in.
...
Lemma le_abs f g : le (abs f) (abs g) = le_fun f g.
  unfold le, le_fun. cbn. fold rk_fun.
  eapply forall_ext.
  move=> [ui vi] Inf.
  have EQ:  lub_list
      (map (fun '(ui0, vi0) => if le' ui0 ui (Init.Nat.max (rk_fun f) (rk_fun g)) then vi0 else bot)
         g) = app g ui.
  2: rewrite EQ; auto.
  unfold app.
  f_equal.
  ...
  rewrite <- rk_enough.
  reflexivity.
  move: (In_rk_fun1 Inf) => h1.
  move: (In_rk_fun1 Ing) => h2.
  lia.
Qed.
...
Lemma le_rk : forall u v, le u v-> rk u <= rk v.
Proof.
  have LEMMA: forall k, forall u v, max (rk u) (rk v) <= k -> le' u v k -> rk u <= rk v.
  2: { intros u v h.  eapply LEMMA; eauto. } 
  elim /strong_ind.
  move=> m ih u v Le h.
  rewrite <- rk_enough in h; auto.
  have LEMMA2: forall f g, le_fun f g -> rk_fun f <= rk_fun g.
  { 
    intros f. 
    induction f. intros g h1. cbn. lia.
    intros g h1. destruct a as [u1 v1].
    destruct g. cbn in h1.
    

  all: destruct u; destruct v; cbn in *; try lia; try done.
  - eapply le_n_S. eapply ih; eauto.
  - fold rk_fun in *.
    eapply le_n_S.
    move: h => /andP [h1 /forallb_forall h2].
